...
describe("source label resolver @unit", () => {
  it("returns source labels for each parsed source type @unit", () => {
    expect(resolveSourceLabel({ type: "local", localPath: "/tmp/skills" })).toBe(
      "/tmp/skills"
    );
    expect(
      resolveSourceLabel({
    ...
    expect(
      resolveSourceLabel({ type: "git", repoUrl: "https://gitlab.com/acme/repo.git" })
    ).toBe("https://gitlab.com/acme/repo.git");
    expect(
      resolveSourceLabel({ type: "well-known", url: "https://example.com/skills" })
    ).toBe("https://example.com/skills");
    expect(
      resolveSourceLabel({ type: "catalog", url: "https://catalog.example.com" })
    ).toBe("https://catalog.example.com");
  });
});
...
describe("remote staging helper @unit", () => {
  it("stages valid remote files with UTF-8 content @unit", () => {
    ...
    try {
      expect(fs.readFileSync(path.join(staged.path, "SKILL.md"), "utf8")).toBe(
        "# Skill\ncontent",
      );
      expect(
        fs.readFileSync(path.join(staged.path, "references/setup.md"), "utf8"),
      ).toBe("setup docs");
    } finally {
      staged.cleanup();
    }
