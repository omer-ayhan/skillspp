...
if rg -n "from ['\"][^'\"]*apps/" packages >/dev/null; then
  echo "Boundary violation: packages must not import from apps." >&2
  rg -n "from ['\"][^'\"]*apps/" packages >&2
  exit 1
fi

# 3) no deep private workspace imports (../..../src) across workspace boundaries
if rg -n "from ['\"]\.\.\/\.\.\/core\/src|from ['\"]\.\.\/\.\.\/platform-node\/src|from ['\"]\.\.\/\.\.\/\.\.\/packages\/" apps packages >/dev/null; then
  echo "Boundary violation: deep private workspace imports are not allowed." >&2
  rg -n "from ['\"]\.\.\/\.\.\/core\/src|from ['\"]\.\.\/\.\.\/platform-node\/src|from ['\"]\.\.\/\.\.\/\.\.\/packages\/" apps packages >&2
  exit 1
fi

# 4) app packages must import explicit core subpaths (not broad root barrel)
if rg -n "from ['\"]@skillspp/core['\"]" apps >/dev/null; then
  echo "Boundary violation: apps must not import @skillspp/core root barrel. Use explicit @skillspp/core/* subpaths." >&2
  rg -n "from ['\"]@skillspp/core['\"]" apps >&2
  exit 1
fi

# 5) core root barrel must not export runtime internals
if rg -n "export \\* from \\\"\\.\\/runtime\\/|export \\* from \\\"\\.\\/sources\\/|export \\* from \\\"\\.\\/providers\\/|export \\* from \\\"\\.\\/contracts\\/runtime-types\\\"|export \\* from \\\"\\.\\/contracts\\/source-types\\\"" packages/core/src/index.ts >/dev/null; then
  echo "Boundary violation: @skillspp/core root barrel exports internal/runtime surfaces." >&2
  rg -n "export \\* from \\\"\\.\\/runtime\\/|export \\* from \\\"\\.\\/sources\\/|export \\* from \\\"\\.\\/providers\\/|export \\* from \\\"\\.\\/contracts\\/runtime-types\\\"|export \\* from \\\"\\.\\/contracts\\/source-types\\\"" packages/core/src/index.ts >&2
  exit 1
fi

# 6) no generated JS/d.ts files in package src directories
if rg --files packages | rg "/src/.*\\.(js|d\\.ts)$" >/dev/null; then
  echo "Boundary violation: generated .js/.d.ts files found under packages/*/src. Build outputs must go to dist/." >&2
  rg --files packages | rg "/src/.*\\.(js|d\\.ts)$" >&2
  exit 1
fi

# 7) forbid importing core intermediate barrels
if rg -n "from ['\"]@skillspp/core/(contracts|runtime|sources)['\"]" apps packages --glob '!**/dist/**' >/dev/null; then
  echo "Boundary violation: do not import @skillspp/core/contracts|runtime|sources barrels; use module-level subpaths." >&2
  rg -n "from ['\"]@skillspp/core/(contracts|runtime|sources)['\"]" apps packages --glob '!**/dist/**' >&2
  exit 1
fi
